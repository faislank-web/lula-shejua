...
def proses_teks_custom(teks):
    ...
    kamus["Download Via"] = "silakan Request di Grup atau di VIP: @ksrfsj_bot"
    footer = "\n\n\nby Channel @Shejua"
...
async def main():
    ...
    try:
        ...
        async for msg in client.iter_messages(SUMBER, min_id=last_id, limit=None, reverse=True):
            ...
            # --- FILTER: Melewati postingan yang mengandung "New Episode Released" ---
            if msg.text and re.search(r'New Episode Released', msg.text, re.IGNORECASE):
                print(f"⏭ Melewati ID {msg.id} (New Episode detected)")
